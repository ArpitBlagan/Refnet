...
const Card = ({ ele, index }: any) => {
  ...
  const rotateX = useTransform(mouseSpringY, [-0.5, 0.5], ['-17.5deg', '17.5deg'])
  const rotateY = useTransform(mouseSpringX, [-0.5, 0.5], ['-17.5deg', '17.5deg'])
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-center w-full">{ele.icon}</div>
        <h1 className="font-thin text-3xl text-white mb-4 relative z-50">{ele.label}</h1>

        <p className="font-normal text-base text-white/40 mb-4 relative z-50">{ele.description}</p>
      </motion.div>
    </motion.div>
  )
}
